...
Al generar o actualizar documentación técnica o de proceso (guides, runbooks, plantillas), aplicar las reglas en `.github/documentation/ORTHOGRAPHY-GUIDELINES.md`. Incluir en el encabezado del documento: "Ortografía verificada según .github/documentation/ORTHOGRAPHY-GUIDELINES.md".
...
## Reglas específicas de este proyecto (RCI Private Area)

### No hardcodear secretos ni configuración sensible en servicios

```typescript
// ❌ Clave hardcodeada en servicio (patrón actualmente presente en EncryptionService)
publicKey = 'MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8A...';

// ✅ Leer desde configuración de entorno
constructor(@Inject(ENCRYPTION_CONFIG) private config: EncryptionConfig) {
  this.publicKey = config.publicKey;
}
// Y definir en environment.ts:
export const environment = { encryptionPublicKey: '...' };
```

### Nombres de clase correctos y sin typos

```typescript
// ❌ Typo en nombre de clase (patrón presente en OneTrustCokkiesService)
export class OneTrustCokkiesService { }  // doble k

// ✅ Nombre correcto
export class OneTrustCookiesService { }  // una sola k
```

### No acceder directamente a localStorage/sessionStorage
...
// ✅ Usar StorageService con claves tipadas
this.storageService.setItem(STORAGE_KEY.TOKEN, value);
this.storageService.getItem(STORAGE_KEY.USER);
```

### No duplicar lógica de journey en componentes

```typescript
// ❌ Llamar al SDK directamente desde un componente
window.tsPlatform.ido.startJourney('my-journey');

// ✅ Usar la fachada Mosaic y el registry de handlers
this.journeyOrchestratorService.start('my-journey');
```

### i18n: centralizar todos los textos

```typescript
// ❌ Texto hardcodeado en componente
this.errorMessage = 'Error al procesar la solicitud';

// ✅ Usar TranslateService con clave en strings-es.json
this.errorMessage = this.translate.instant('errors.process_request');
// Y añadir la clave en src/assets/i18n/strings-es.json
```
